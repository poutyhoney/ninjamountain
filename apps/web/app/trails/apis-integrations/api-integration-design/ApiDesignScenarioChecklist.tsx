...
const CHECKS = [
  { key: 'wrong',   label: 'Identify what is wrong with the "before" endpoint design' },
  { key: 'fix',     label: 'Identify which specific change fixes each problem' },
  { key: 'paging',  label: 'Determine what pagination strategy the "after" version should use, and why' },
  { key: 'errors',  label: 'Confirm the error response shape is consistent across endpoints' },
  { key: 'more',    label: 'Describe one more design choice you would want locked in before this API ships' },
] as const;

type CheckKey = (typeof CHECKS)[number]['key'];
type CheckedState = Record<CheckKey, boolean>;

const storageKey = (key: CheckKey) => `webdev-apidesign-lesson-${key}`;

// All unchecked: the server render and the pre-hydration client render.
const SERVER_SNAPSHOT: CheckedState = {
  wrong: false, fix: false, paging: false, errors: false, more: false,
};
...
export default function ApiDesignScenarioChecklist() {
  ...
  return (
    <div className="rounded-[18px] border border-[#202431] bg-[#151821] p-6 shadow-[0_18px_55px_rgba(0,0,0,0.23)]">
      ...
      <p className="mb-4 text-sm text-[#6F7684]">
        Work through the sample design review and check each item off. Progress is saved in your browser.
      </p>
      <div>
