...
const CHECKS = [
  { key: 'slow',    label: 'Identify what the original query is doing and why it is slow' },
  { key: 'column',  label: 'Identify the column that needs an index based on the WHERE clause' },
  { key: 'confirm', label: 'Confirm the improvement an index would make, and why' },
  { key: 'downside',label: 'Determine whether adding this index could have any downside' },
  { key: 'verify',  label: 'Describe how you would verify the fix before and after in production' },
] as const;

type CheckKey = (typeof CHECKS)[number]['key'];
type CheckedState = Record<CheckKey, boolean>;

const storageKey = (key: CheckKey) => `webdev-sqlbasics-lesson-${key}`;

// All unchecked: the server render and the pre-hydration client render.
const SERVER_SNAPSHOT: CheckedState = {
  slow: false, column: false, confirm: false, downside: false, verify: false,
};
...
export default function SqlBasicsScenarioChecklist() {
  ...
  return (
    <div className="rounded-[18px] border border-[#202431] bg-[#151821] p-6 shadow-[0_18px_55px_rgba(0,0,0,0.23)]">
      ...
      <p className="mb-4 text-sm text-[#6F7684]">
        Work through the sample slow query and check each item off. Progress is saved in your browser.
      </p>
      <div>
