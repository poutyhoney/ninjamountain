...
const CHECKS = [
  { key: 'toolCall',  label: 'Identify which tool call actually happened and with what argument' },
  { key: 'origin',    label: 'Determine whether that argument came from a real lookup or was invented' },
  { key: 'missing',   label: 'Identify what tool call (if any) should have happened first' },
  { key: 'guardrail', label: 'Describe what guardrail would have caught this before it reached the refund tool' },
  { key: 'eval',      label: 'Describe the eval you would add so this exact failure does not ship again' },
] as const;

type CheckKey = (typeof CHECKS)[number]['key'];
type CheckedState = Record<CheckKey, boolean>;

const storageKey = (key: CheckKey) => `aiml-agentic-lesson-${key}`;

// All unchecked: the server render and the pre-hydration client render.
const SERVER_SNAPSHOT: CheckedState = {
  toolCall: false, origin: false, missing: false, guardrail: false, eval: false,
};
...
export default function AgenticScenarioChecklist() {
  ...
  return (
    <div className="rounded-[18px] border border-[#202431] bg-[#151821] p-6 shadow-[0_18px_55px_rgba(0,0,0,0.23)]">
      ...
      <p className="mb-4 text-sm text-[#6F7684]">
        Work through the sample agent trace and check each item off. Progress is saved in your browser.
      </p>
      <div>
