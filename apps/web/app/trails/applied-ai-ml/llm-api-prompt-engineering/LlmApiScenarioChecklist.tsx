...
const CHECKS = [
  { key: 'systemRole', label: 'Identify what the system message is constraining' },
  { key: 'userRole',   label: 'Identify what the user message actually is' },
  { key: 'valid',      label: 'Confirm the response is valid, parseable JSON matching the requested shape' },
  { key: 'downstream', label: 'Identify what would happen downstream if the model returned prose instead of JSON' },
  { key: 'verify',     label: 'Describe how you would validate this response before trusting it in production' },
] as const;

type CheckKey = (typeof CHECKS)[number]['key'];
type CheckedState = Record<CheckKey, boolean>;

const storageKey = (key: CheckKey) => `aiml-llmapi-lesson-${key}`;

// All unchecked: the server render and the pre-hydration client render.
const SERVER_SNAPSHOT: CheckedState = {
  systemRole: false, userRole: false, valid: false, downstream: false, verify: false,
};
...
export default function LlmApiScenarioChecklist() {
