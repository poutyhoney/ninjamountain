...
const CHECKS = [
  { key: 'localDig',  label: 'Run `dig` against the local resolver and note the returned IP' },
  { key: 'publicDig', label: 'Run `dig` against a public resolver (e.g. 8.8.8.8) and compare' },
  { key: 'curl',      label: 'Attempt a `curl -v` and note whether it times out or is refused' },
  { key: 'diagnose',  label: 'Determine whether this is a DNS propagation issue or a service-down issue' },
  { key: 'fix',       label: 'Describe the fix or next diagnostic step' },
] as const;

type CheckKey = (typeof CHECKS)[number]['key'];
type CheckedState = Record<CheckKey, boolean>;

const storageKey = (key: CheckKey) => `devops-cloudops-lesson-${key}`;

// All unchecked: the server render and the pre-hydration client render.
const SERVER_SNAPSHOT: CheckedState = {
  localDig: false, publicDig: false, curl: false, diagnose: false, fix: false,
};
...
export default function CloudOpsScenarioChecklist() {
  ...
  return (
    <div className="rounded-[18px] border border-[#202431] bg-[#151821] p-6 shadow-[0_18px_55px_rgba(0,0,0,0.23)]">
      ...
      <p className="mb-4 text-sm text-[#6F7684]">
        Work through the sample dig/curl output and check each item off. Progress is saved in your browser.
      </p>
      <div>
