...
const CHECKS = [
  { key: 'reason',    label: 'Identify the failure reason from `kubectl describe pod`' },
  { key: 'restarts',  label: 'Check the restart count and the last-terminated reason' },
  { key: 'logs',      label: 'Pull logs from the previous (crashed) container, not the current one' },
  { key: 'rootCause', label: 'Determine whether the crash is application-level or infra-level' },
  { key: 'fix',       label: 'Describe the fix or the next diagnostic step you would take' },
] as const;

type CheckKey = (typeof CHECKS)[number]['key'];
type CheckedState = Record<CheckKey, boolean>;

const storageKey = (key: CheckKey) => `devops-containers-lesson-${key}`;

// All unchecked: the server render and the pre-hydration client render.
const SERVER_SNAPSHOT: CheckedState = {
  reason: false, restarts: false, logs: false, rootCause: false, fix: false,
};
...
export default function ContainersScenarioChecklist() {
  ...
  return (
    <div className="rounded-[18px] border border-[#202431] bg-[#151821] p-6 shadow-[0_18px_55px_rgba(0,0,0,0.23)]">
      ...
      <p className="mb-4 text-sm text-[#6F7684]">
        Work through the sample kubectl output and check each item off. Progress is saved in your browser.
      </p>
      <div>
