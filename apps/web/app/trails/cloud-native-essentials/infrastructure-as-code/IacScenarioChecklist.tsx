...
const CHECKS = [
  { key: 'attribute', label: 'Read the plan output and identify which attribute changed' },
  { key: 'replace',   label: 'Determine whether the change is in-place or forces replacement' },
  { key: 'meaning',   label: 'Explain what "forces replacement" means for this resource' },
  { key: 'cause',     label: 'Identify who or what likely caused the drift' },
  { key: 'fix',       label: 'Describe the fix or the process change that prevents this next time' },
] as const;

type CheckKey = (typeof CHECKS)[number]['key'];
type CheckedState = Record<CheckKey, boolean>;

const storageKey = (key: CheckKey) => `devops-iac-lesson-${key}`;

// All unchecked: the server render and the pre-hydration client render.
const SERVER_SNAPSHOT: CheckedState = {
  attribute: false, replace: false, meaning: false, cause: false, fix: false,
};
...
export default function IacScenarioChecklist() {
  ...
  return (
    <div className="rounded-[18px] border border-[#202431] bg-[#151821] p-6 shadow-[0_18px_55px_rgba(0,0,0,0.23)]">
      ...
      <p className="mb-4 text-sm text-[#6F7684]">
        Work through the sample plan output and check each item off. Progress is saved in your browser.
      </p>
      <div>
