...
const CHECKS = [
  { key: 'which',   label: 'Identify which task is failing and on which dates' },
  { key: 'cause',   label: 'Identify the actual error message or cause for the failure' },
  { key: 'alert',   label: 'Determine why nobody was alerted sooner' },
  { key: 'fix',     label: 'Describe the fix for the pipeline itself' },
  { key: 'backfill',label: 'Describe the backfill needed to recover the missed days' },
] as const;

type CheckKey = (typeof CHECKS)[number]['key'];
type CheckedState = Record<CheckKey, boolean>;

const storageKey = (key: CheckKey) => `data-pipelines-lesson-${key}`;

// All unchecked: the server render and the pre-hydration client render.
const SERVER_SNAPSHOT: CheckedState = {
  which: false, cause: false, alert: false, fix: false, backfill: false,
};
...
export default function PipelinesScenarioChecklist() {
  ...
  return (
    <div className="rounded-[18px] border border-[#202431] bg-[#151821] p-6 shadow-[0_18px_55px_rgba(0,0,0,0.23)]">
      ...
      <p className="mb-4 text-sm text-[#6F7684]">
        Work through the sample DAG run history and check each item off. Progress is saved in your browser.
      </p>
      <div>
