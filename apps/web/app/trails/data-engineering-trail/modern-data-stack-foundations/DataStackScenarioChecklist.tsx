...
const CHECKS = [
  { key: 'naming',   label: 'Identify the naming convention used for the tables in the query' },
  { key: 'factDim',  label: 'Determine which table holds facts/events vs. descriptive attributes' },
  { key: 'joinKey',  label: 'Confirm the join key connecting the two tables' },
  { key: 'question', label: 'Identify what business question this query is actually answering' },
  { key: 'verify',   label: 'Describe how you would verify the result before reporting it as an answer' },
] as const;

type CheckKey = (typeof CHECKS)[number]['key'];
type CheckedState = Record<CheckKey, boolean>;

const storageKey = (key: CheckKey) => `data-stack-lesson-${key}`;

// All unchecked: the server render and the pre-hydration client render.
const SERVER_SNAPSHOT: CheckedState = {
  naming: false, factDim: false, joinKey: false, question: false, verify: false,
};
...
export default function DataStackScenarioChecklist() {
  ...
  return (
    <div className="rounded-[18px] border border-[#202431] bg-[#151821] p-6 shadow-[0_18px_55px_rgba(0,0,0,0.23)]">
      ...
      <p className="mb-4 text-sm text-[#6F7684]">
        Work through the sample query and check each item off. Progress is saved in your browser.
      </p>
      <div>
