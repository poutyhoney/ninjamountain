...
const CHECKS = [
  { key: 'window',   label: 'Identify the service and time window common to both alerts' },
  { key: 'logLine',  label: 'Find the log line(s) matching that service and time window' },
  { key: 'traceId',  label: 'Extract the trace_id from the matching log line' },
  { key: 'cause',    label: "Determine what the log line's error message suggests as a root cause" },
  { key: 'next',     label: 'Describe the next diagnostic step' },
] as const;

type CheckKey = (typeof CHECKS)[number]['key'];
type CheckedState = Record<CheckKey, boolean>;

const storageKey = (key: CheckKey) => `data-observability-lesson-${key}`;

// All unchecked: the server render and the pre-hydration client render.
const SERVER_SNAPSHOT: CheckedState = {
  window: false, logLine: false, traceId: false, cause: false, next: false,
};
...
export default function ObservabilityScenarioChecklist() {
  ...
  return (
    <div className="rounded-[18px] border border-[#202431] bg-[#151821] p-6 shadow-[0_18px_55px_rgba(0,0,0,0.23)]">
      ...
      <p className="mb-4 text-sm text-[#6F7684]">
        Work through the sample log line and alerts and check each item off. Progress is saved in your browser.
      </p>
      <div>
