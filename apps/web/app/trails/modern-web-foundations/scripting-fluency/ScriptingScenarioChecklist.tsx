...
const CHECKS = [
  { key: 'bash',    label: 'Identify what the Bash version actually does, line by line' },
  { key: 'python',  label: 'Identify what the Python version does the same, and what it adds' },
  { key: 'ci',      label: 'Decide which you would reach for if this needed to run in a CI pipeline daily' },
  { key: 'reuse',   label: 'Decide which you would reach for if a teammate needed to reuse and modify it' },
  { key: 'edge',    label: 'Describe one edge case the Bash version would mishandle that Python handles explicitly' },
] as const;

type CheckKey = (typeof CHECKS)[number]['key'];
type CheckedState = Record<CheckKey, boolean>;

const storageKey = (key: CheckKey) => `webdev-scripting-lesson-${key}`;

// All unchecked: the server render and the pre-hydration client render.
const SERVER_SNAPSHOT: CheckedState = {
  bash: false, python: false, ci: false, reuse: false, edge: false,
};
...
export default function ScriptingScenarioChecklist() {
  ...
  return (
    <div className="rounded-[18px] border border-[#202431] bg-[#151821] p-6 shadow-[0_18px_55px_rgba(0,0,0,0.23)]">
      ...
      <p className="mb-4 text-sm text-[#6F7684]">
        Work through the sample comparison and check each item off. Progress is saved in your browser.
      </p>
      <div>
