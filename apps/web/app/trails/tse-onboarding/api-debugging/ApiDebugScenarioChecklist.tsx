...
const CHECKS = [
  { key: 'reconstruct', label: 'Reconstruct the actual request being sent (method, path, headers, body)' },
  { key: 'reproduce',   label: 'Reproduce it in Postman against the same environment the customer used' },
  { key: 'readFull',    label: 'Read the full response, not just the status code' },
  { key: 'diagnose',    label: 'Identify whether this is rate limiting, an auth issue, or an application error' },
  { key: 'fix',         label: 'Describe the fix or the guidance you would give the customer' },
] as const;

type CheckKey = (typeof CHECKS)[number]['key'];
type CheckedState = Record<CheckKey, boolean>;

const storageKey = (key: CheckKey) => `tse-api-debugging-lesson-${key}`;

// All unchecked: the server render and the pre-hydration client render.
const SERVER_SNAPSHOT: CheckedState = {
  reconstruct: false, reproduce: false, readFull: false, diagnose: false, fix: false,
};
...
export default function ApiDebugScenarioChecklist() {
  ...
  return (
    <div className="rounded-[18px] border border-[#202431] bg-[#151821] p-6 shadow-[0_18px_55px_rgba(0,0,0,0.23)]">
      ...
      <p className="mb-4 text-sm text-[#6F7684]">
        Work through the sample request/response and check each item off. Progress is saved in your browser.
      </p>
      <div>
