...
const CHECKS = [
  { key: 'scheme',  label: 'Identify the authentication scheme used (Bearer, Basic, API key, etc.)' },
  { key: 'decode',  label: 'Decode the token and inspect its header and payload claims' },
  { key: 'expiry',  label: 'Check the expiry claim against the current time' },
  { key: 'scopes',  label: 'Check whether the token’s scopes cover the endpoint being called' },
  { key: 'verdict', label: 'Decide: identity failure (401) or permission failure (403)?' },
] as const;

type CheckKey = (typeof CHECKS)[number]['key'];
type CheckedState = Record<CheckKey, boolean>;

const storageKey = (key: CheckKey) => `tse-auth-lesson-${key}`;

// All unchecked: the server render and the pre-hydration client render.
const SERVER_SNAPSHOT: CheckedState = {
  scheme: false, decode: false, expiry: false, scopes: false, verdict: false,
};
...
export default function AuthScenarioChecklist() {
