...
const CHECKS = [
  { key: 'attempts',  label: 'Confirm the event ID and how many delivery attempts were logged' },
  { key: 'response',  label: 'Check the response code and timing your endpoint returned on each attempt' },
  { key: 'dedupe',    label: 'Verify your endpoint deduplicates by event ID rather than accepting blindly' },
  { key: 'ordering',  label: 'Check whether your handler wrongly assumes strict event ordering' },
  { key: 'signature', label: 'Confirm the signature header was validated before processing the payload' },
] as const;

type CheckKey = (typeof CHECKS)[number]['key'];
type CheckedState = Record<CheckKey, boolean>;

const storageKey = (key: CheckKey) => `tse-events-lesson-${key}`;

// All unchecked: the server render and the pre-hydration client render.
const SERVER_SNAPSHOT: CheckedState = {
  attempts: false, response: false, dedupe: false, ordering: false, signature: false,
};
...
export default function EventsScenarioChecklist() {
  ...
  return (
    <div className="rounded-[18px] border border-[#202431] bg-[#151821] p-6 shadow-[0_18px_55px_rgba(0,0,0,0.23)]">
      ...
      <p className="mb-4 text-sm text-[#6F7684]">
        Work through the sample delivery log and check each item off. Progress is saved in your browser.
      </p>
      <div>
