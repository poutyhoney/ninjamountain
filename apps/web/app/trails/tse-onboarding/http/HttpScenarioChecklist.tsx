...
const CHECKS = [
  { key: 'method',  label: 'Identify the HTTP method used in the request' },
  { key: 'status',  label: 'Identify the status code in the response' },
  { key: 'meaning', label: 'Explain what that status code means, in your own words' },
  { key: 'cause',   label: 'Name the most likely cause based on the response body' },
...
const storageKey = (key: CheckKey) => `tse-http-lesson-${key}`;

// All unchecked: the server render and the pre-hydration client render.
const SERVER_SNAPSHOT: CheckedState = {
  method: false, status: false, meaning: false, cause: false, fix: false,
};
...
export default function HttpScenarioChecklist() {
