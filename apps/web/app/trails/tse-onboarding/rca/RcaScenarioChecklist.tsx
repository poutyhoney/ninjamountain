...
const CHECKS = [
  { key: 'detection', label: 'Identify how long it took to detect the issue after the deploy' },
  { key: 'response',  label: 'Identify how long it took to engage the right responder' },
  { key: 'rootcause', label: 'Separate the root cause from the contributing factor(s)' },
  { key: 'severity',  label: 'Confirm the severity reflects customer impact, not root cause' },
  { key: 'action',    label: 'Write one corrective action with a specific owner and target date' },
] as const;

type CheckKey = (typeof CHECKS)[number]['key'];
type CheckedState = Record<CheckKey, boolean>;

const storageKey = (key: CheckKey) => `tse-rca-lesson-${key}`;

// All unchecked: the server render and the pre-hydration client render.
const SERVER_SNAPSHOT: CheckedState = {
  detection: false, response: false, rootcause: false, severity: false, action: false,
};
...
export default function RcaScenarioChecklist() {
  ...
  return (
    <div className="rounded-[18px] border border-[#202431] bg-[#151821] p-6 shadow-[0_18px_55px_rgba(0,0,0,0.23)]">
      ...
      <p className="mb-4 text-sm text-[#6F7684]">
        Work through the sample incident timeline and check each item off. Progress is saved in your browser.
      </p>
      <div>
