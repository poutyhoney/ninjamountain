...
const CHECKS = [
  { key: 'scope',    label: "Identify what's actually failing — all users at once vs. one user" },
  { key: 'pattern',  label: 'Check whether this matches a known SSO failure pattern (cert rotation, clock skew, SCIM lag, mapping mismatch)' },
  { key: 'confirm',  label: 'Confirm the IdP-side change that lines up with the timing' },
  { key: 'classify', label: 'Distinguish an authentication failure from a provisioning/permissions failure' },
  { key: 'fix',      label: 'Describe the fix and who owns it — the customer\'s IdP admin or your team' },
] as const;

type CheckKey = (typeof CHECKS)[number]['key'];
type CheckedState = Record<CheckKey, boolean>;

const storageKey = (key: CheckKey) => `tse-sso-lesson-${key}`;

// All unchecked: the server render and the pre-hydration client render.
const SERVER_SNAPSHOT: CheckedState = {
  scope: false, pattern: false, confirm: false, classify: false, fix: false,
};
...
export default function SsoScenarioChecklist() {
  ...
  return (
    <div className="rounded-[18px] border border-[#202431] bg-[#151821] p-6 shadow-[0_18px_55px_rgba(0,0,0,0.23)]">
      ...
      <p className="mb-4 text-sm text-[#6F7684]">
        Work through the sample failure log and check each item off. Progress is saved in your browser.
      </p>
      <div>
