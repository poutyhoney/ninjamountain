...
const CHECKS = [
  { key: 'complaint', label: 'Identify the customer-facing ticket and its actual complaint' },
  { key: 'engTicket', label: 'Follow the linked engineering ticket and note its status' },
  { key: 'flag',       label: 'Check whether a feature flag explains the discrepancy' },
  { key: 'diagnose',   label: 'Determine whether this is a bug, a flag/rollout state, or expected behavior' },
  { key: 'reply',      label: 'Describe what you would tell the customer, referencing what you found' },
] as const;

type CheckKey = (typeof CHECKS)[number]['key'];
type CheckedState = Record<CheckKey, boolean>;

const storageKey = (key: CheckKey) => `tse-toolbelt-lesson-${key}`;

// All unchecked: the server render and the pre-hydration client render.
const SERVER_SNAPSHOT: CheckedState = {
  complaint: false, engTicket: false, flag: false, diagnose: false, reply: false,
};
...
export default function ToolbeltScenarioChecklist() {
  ...
  return (
    <div className="rounded-[18px] border border-[#202431] bg-[#151821] p-6 shadow-[0_18px_55px_rgba(0,0,0,0.23)]">
      ...
      <p className="mb-4 text-sm text-[#6F7684]">
        Work through the sample ticket trace and check each item off. Progress is saved in your browser.
      </p>
      <div>
